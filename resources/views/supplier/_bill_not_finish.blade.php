...
<div class="row">
  <table class="table table-bordered table-striped table-hover" id="data-table">
    ...
    <tbody>
      @php $x = count($bill_not_finishs); @endphp
      @foreach ($bill_not_finishs as $value)
        <tr>
        <td>{{ $x }}</td>
        <td class="invoice_no">{{ $value->invoice_no }}</td>
        <td class="supplier"><a href="{{ url('suppliers/'.$value->supplier->id) }}">{{ $value->supplier->name }}</a></td>
